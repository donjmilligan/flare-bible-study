...
router.get("/messageofhope", async (req, res) => {
  try {
    const query = "SELECT id, name, imports FROM message_hope";
...
router.post("/messageofhope", async (req, res) => {
  ...
  try {
    const query =
      "INSERT INTO message_hope (name, imports) VALUES ($1, $2) RETURNING *";
...
router.delete("/messageofhope/:name", async (req, res) => {
  const { name } = req.params;
  if (!name) {
    return res.status(400).json({ success: false, error: "Name is required" });
  }
  try {
    const query = "DELETE FROM message_hope WHERE name = $1";
...
router.patch("/messageofhope/:name", async (req, res) => {
  ...
  try {
    const query =
      "UPDATE message_hope SET angle = $1, radius = $2 WHERE name = $3 RETURNING *";
