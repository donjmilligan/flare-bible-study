...
const MessageofHope = () => {
  ...
  // Fetch data
  useEffect(() => {
    fetch("http://localhost:3001/api/bible/messageofhope")
      .then((res) => res.json())
  ...
  const handleRenameSubmit = async () => {
    ...
    try {
      // Delete the old node
      await fetch(
        `http://localhost:3001/api/bible/messageofhope/${encodeURIComponent(currentName)}`,
        {
          method: "DELETE",
        },
      );
      ...
      await fetch("http://localhost:3001/api/bible/messageofhope", {
        method: "POST",
      ...
      const response = await fetch(
        "http://localhost:3001/api/bible/messageofhope",
      );
  ...
  const handleDelete = async (node) => {
    ...
    await fetch(
      `http://localhost:3001/api/bible/messageofhope/${encodeURIComponent(node.data.name)}`,
      {
        method: "DELETE",
      },
    );
        ...
        >
          <form
            onSubmit={(e) => {
              ...
              fetch("http://localhost:3001/api/messageofhope", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  name: fullName,
