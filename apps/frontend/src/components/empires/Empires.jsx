...
const MessageofHope = () => {
  ...
  // Fetch data
  useEffect(() => {
    fetch("http://localhost:3001/api/bible/empires")
      .then((res) => res.json())
  ...
  const handleRenameSubmit = async () => {
    ...
    try {
      // Delete the old node
      await fetch(
        `http://localhost:3001/api/bible/empires/${encodeURIComponent(currentName)}`,
        {
          method: "DELETE",
        },
      );
      ...
      await fetch("http://localhost:3001/api/bible/empires", {
        method: "POST",
      ...
      // Refresh data
      const response = await fetch("http://localhost:3001/api/bible/empires");
  ...
  const handleDelete = async (node) => {
    ...
    await fetch(
      `http://localhost:3001/api/bible/empires/${encodeURIComponent(node.data.name)}`,
      {
        method: "DELETE",
      },
    );
