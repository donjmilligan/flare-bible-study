...
const OldTestamentJesus2 = () => {
  ...
  // Fetch data
  useEffect(() => {
    fetch("http://localhost:3001/api/bible/oldtestamentjesus2")
      .then((res) => res.json())
  ...
  const handleRenameSubmit = async () => {
    ...
    try {
      // Delete the old node
      await fetch(
        `http://localhost:3001/api/bible/oldtestamentjesus2/${encodeURIComponent(currentName)}`,
        {
          method: "DELETE",
        },
      );
      ...
      await fetch("http://localhost:3001/api/bible/oldtestamentjesus2", {
        method: "POST",
      ...
      const response = await fetch(
        "http://localhost:3001/api/bible/oldtestamentjesus2",
      );
  ...
  const handleDelete = async (node) => {
    ...
    await fetch(
      `http://localhost:3001/api/bible/oldtestamentjesus2/${encodeURIComponent(node.data.name)}`,
      {
        method: "DELETE",
      },
    );
...
export default OldTestamentJesus2;
