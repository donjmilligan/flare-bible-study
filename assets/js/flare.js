...
d3.json('data/kjv.json', function (err, json) {
    ...
    // get width of parent and set height for sacaling
    var width = d3.select('#bible-chart').style('width'),
        height,
        scale;
    ...
    scale = (width / 1300);
    //console.log(width + ':w + h:' + height + " sc:" + scale);
    
    /* SVG  SVG  */
    var svg = d3.select('#paradoxes-chart');
    ...
    if ($(window).width() < 1024){
        g.transition()
          .call(zoom.transform, transform(0));
    ...
    */ 
    ...
    d3.select('#color-select')
        .on('change', function () {
            ...
            d3.select('#paradoxes-chart').selectAll('.arc').remove();
            // Set the filter
