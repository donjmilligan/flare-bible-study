...
import '/OldTestamentsJesus1.css';
import InfoPanel from '../WorldEmpires/InfoPanel.tsx';


const OldTestamentsJesus1 = ({ textSize, circleSize, onNodeClick }) => {
  const svgRef = useRef();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3000/api/oldtestamentsjesus1');
  ...
  const addNewNode = async () => {
    if (newNodeData.name) {
      try {
        const response = await fetch('http://localhost:3000/oldtestamentsjesus1', {
          method: 'POST',
  ...
  const saveNodeEdit = async () => {
    if (editingNode && newNodeData.name) {
      try {
        ...
        const updatedNode = editingNode; // No-op for now
  ...
  const handleInlineRename = async (e) => {
    ...
    try {
      ...
      // if (!response.ok) throw new Error('Failed to update node');
      // const updatedNode = await response.json();
      const updatedNode = inlineEdit.node; // No-op for now
      const updatedSource = source.map(node =>
...
export default OldTestamentsJesus1; 