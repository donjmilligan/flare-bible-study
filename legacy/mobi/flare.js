...
var para = null;
var paraFilters = {
  source: window._paraSource
    ? window._paraSource
    : "sab" /* Source of data, default SAB */,
...
function bibleFromSQL(str) {
  ...
  if (str == "") {
    ...
    xmlhttp.open(
      "GET",
      "./sql/ajax.php?b=" + str + "&t=" + paraFilters.translation,
      true,
    );
    xmlhttp.send();
    setTimeout(function () {
...
function test(config) {
  ...
  var xScale = d3.time
    .scale()
    .domain([d3.time.year.offset(current, -1), current]);
  ...
  var result = function (selection) {
    selection.each(function (data) {
      ...
      rectItem
        ...
        .attr("width", xScale(d3.time.day.offset(xScale.invert(0), 7)))
        .attr("height", height);
...
d3.json("assets/json/kjv.json", function (err, json) {
  ...
  for (var x = 0; x < json.sections.length; x++) {
    for (var y = 0; y < json.sections[x].books.length; y++) {
      bookSelect.append("option").text(json.sections[x].books[y].shortName);
  ...
  if ($(window).width() < 1024) {
    g.transition().call(zoom.transform, transform(0.02));
  } else if ($(window).width() < 1700 && $(window).width() >= 1024) {
    g.transition().call(zoom.transform, transform(0.12));
  }

  function transform(e) {
    e = scale + e;
    return d3.zoomIdentity.translate(25, 0).scale(e);
  }
  ...
  typeSelect
    .selectAll("option")
    .data(Object.keys(paraTypeFilters))
    .enter()
    .append("option")
    .text(function (d) {
      return d;
  ...
  d3.select("#translation-select").on("change", function () {
  ...
  d3.select("#text-search").on("keyup", function () {
    var text = this.value;
...
function renderCompare() {
  ...
  var chart = d3
    ...
    .selectAll(".arc")
    .data(
      para[paraFilters.source].paradoxes.filter(function (d) {
        ...
        if (paraFilters.book !== null) {
          ...
          for (i = 0; i < Math.min(refList.length - 1, 10); i++) {
            ...
            if (
              match &&
              (match[1] == paraFilters.book ||
                match[1] + "s" == paraFilters.book)
            ) {
              found = true;
              break;
            }
  ...
  chart
    ...
    .on("mouseover", function (d) {
      ...
      d3.select("#thread-content").html(
        '<span class="subdued">' +
          refList.join(", ").substr(0, maxLength) +
          "</span>",
      );
    })
    .on("mouseout", function (d) {
        ...
        for (x = 0; x <= Math.min(maxArcs, refList.length - 2); x++) {
          ...
          if (!isNaN(start) && !isNaN(end) && !isNaN(r) && !isNaN(ry)) {
            ...
            group
              ...
              .on("click", function (d) {
                var str =
                  d3.select(this).attr("start") +
                  "," +
                  d3.select(this).attr("end");
                bibleFromSQL(str);
              })
              .style(
                "stroke",
  ...
  chart.exit().transition().duration(350).style("opacity", 0).remove();
  /* end define chart arcs */

  /* Update any highlighting from filter controllers */
  d3.select("#arc-chart g").selectAll("rect").classed("selected", false);
...
function newTab() {
  return (
    window.event &&
    ((event.which == 1 && (event.ctrlKey === true || event.metaKey === true)) ||
      event.which == 2)
  );
}
